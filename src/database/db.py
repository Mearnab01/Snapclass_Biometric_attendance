...
def get_attendance_for_subject(subject_id: int):
    ...
    enrolled_ids = {row['student_id'] for row in enrolled_res.data}
    ...
    students_map = {s['student_id']: s for s in students_res.data}

    # attach student — only include logs for currently enrolled students
    result = []
    for log in logs_res.data:
        if log['student_id'] in enrolled_ids:
            log['students'] = students_map.get(log['student_id'], {})
            result.append(log)
